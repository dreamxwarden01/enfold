...
no HMAC fold — so the vectors are regenerated and the clean-room check is done again before the
format is frozen. The path from `H` to `IK` is not a standard construction:
