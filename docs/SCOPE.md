...
`FORMAT.md` §3.3 alone (63 of 63 values identical). Reopened by `FORMAT.md` Revision 2
(2026-09-07) — the hardware chain changed: `K_P` from the password alone, `H ‖ K_P` into HKDF,
no HMAC fold — and **closed again on 2026-09-08**: the vectors were regenerated, the unchanged
subtrees diffed byte for byte against the pre-R2 file, and the changed subtree (`vault_salt'`,
`K_P`, the entangle IKM, `pre` and `IK` per case incl. the 512 MiB anchor, `KWK_secrets`, the
three secrets records) re-derived by a second clean-room implementation from §3, §3.3 and §7.6
alone: 39 of 39 values identical, two sentences added to the text where it had to guess
(DECISIONS 2026-09-08). The path from `H` to `IK` is not a standard construction:
Argon2id over the password, then HKDF over `H ‖ K_P`, with a conditional branch when no password
is set, and a hybrid variant for software slots. Nothing else will catch a transposed argument or a wrong
