...
// The tree the page reads (APP.md §2.3): the committed snapshot — the
// archive's Files() and Dirs() together, re-taken after every commit — held
// as the tree FORMAT.md R39 describes. A directory is a record with an id, a
// file hangs off its parent by id, and the root is the all-zero id, which
// never has a record. Nothing here is derived from a path and no folder is
// inferred from a name (DESIGN.md trap 31): the page's rows, its breadcrumb,
// every pre-flight and the extraction plan all read this tree.
//
// Since 2026-09-09 there is no overlay and no pending vocabulary: each
// operation is its own transaction, committed at its end, so a row is
// committed or it is not listed at all (APP.md §2.3, DECISIONS 2026-09-09).

// mergedRec is one record of that view.
...
// merge builds the view from the committed snapshot. Caller holds the state
// mutex. A record reachable from nowhere — which a well-formed index has none
// of — is simply not in the view.
func (oa *openArchive) merge() *merged {
	recs := make([]*mergedRec, 0, len(oa.dirSnap)+len(oa.snap))
	for i := range oa.dirSnap {
		d := &oa.dirSnap[i]
		recs = append(recs, &mergedRec{
...
// rec finds a record of the view.
func (m *merged) rec(id [16]byte) *mergedRec { return m.byID[id] }

// live is the record a call may act on. The root is never a record (APP.md
// §3), so it is never live.
func (m *merged) live(id [16]byte) *mergedRec { return m.byID[id] }

// dirUsable reports whether id may be listed into, or given records: the
// root, or a directory of the view (APP.md §3).
...
// false when id is not the root or a directory of the view.
...
// record the walk has not created yet is measured the same way.
...
// its first entry is the root under the archive's name and its last is dirID
// itself, so the page draws the whole breadcrumb from this alone (APP.md §3).
...
// subtree, which is what a folder row's Size shows (APP.md §3).
